...
<head>
  ...
  <title>Wellness - Create New Hctonee</title>
  ...
  <?php
	...
	if (isset($_POST['sub1'])){
		...
		$hctonee = $_POST['hctonee'];
		$name = $_POST['name'];
		$type = $_POST['type'];
		$bval = $_POST['bval'];
		//Clean input
		$hctonee = $obj1->cleaninput($hctonee);
		...
		$hctoneesta = $obj1->vhctonee($hctonee, $constypeid);
		if ($hctoneesta != ""){
			$e++;
		}
		...
		if ($e == 0){
			...
			$res1 = $obj1->crhctonee1($constypeid, $hctonee, $name, $type, $bval);
			if ($res1){
				...
				$hctoneeid = $obj1->getlastinsertid();
				$res2 = $obj1->uphctoneelog($uname, $ipadd, $dts, $action, $hctoneeid);
				if ($res2){
					$outsta = "Hctonee created successfully";
				}
				else{
		...
		<div class="twelve columns" id="container1a">
			...
			<div class="row">
				<div class="twelve columns">
					<form name="frmtopic" id="topicfrom" action="crhctonee.php" method="post">
						<fieldset>
							...
							<div class="row">
								...
								<div class="four columns">
									<input type="text" name="hctonee" id="txt1" tabindex="2" maxlength="90"/>
								</div>
								<div class="five columns">
									<?php print $hctoneesta; ?>
								</div>
							</div>
							<div class="row">
								<div class="three columns">
