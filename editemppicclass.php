...
include "matt.php";
    class editemppic extends db{
		...
		public function editemppic1($empid, $destination){
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					$tex = $this->tex($tablename);
					if ($tex){
						$sql = "UPDATE `$this->db`.`$tablename` SET `$tablename`.`imgpath` = '$destination' WHERE `$tablename`.`id` = '$empid'";
						$result = mysql_query($sql, $dbhandle);
						if ($result){
							$ret = TRUE;
						}
