<?php
    ...
    class consperemp extends db{
		...
		public function getempconsnumbers($clid, $locid, $sex, $arl, $arh, $year, $month){
			$ret = array();
			
			$conscountarr = $this->getempconscountarr($clid, $locid, $sex, $arl, $arh, $year, $month);
			
			$hcn = $this->gethccnumbers($conscountarr);
			$icn = $this->geticcnumbers($conscountarr);
			$ocn = $this->getoccnumbers($conscountarr);
			$pcn = $this->getpccnumbers($conscountarr);
			
			$ret = array($hcn, $icn, $ocn, $pcn);
			
			return $ret;
		}
		
		public function gethccnumbers($conscountarr){
			$ret = array();
		...
		public function geticcnumbers($conscountarr){
			$ret = array();
		...
		public function getoccnumbers($conscountarr){
			$ret = array();
		...
		public function getpccnumbers($conscountarr){
			$ret = array();
		...
		public function getempconscountarr($clid, $locid, $sex, $arl, $arh, $year, $month){
			$ret = array();
			
			$conarr = $this->getempconarr($clid, $locid, $sex, $arl, $arh, $year, $month);
			
			$empconsarr = $this->getempcons($conarr);
			
			$empconscount = array();
			
			$i = 0;
			
			foreach ($empconsarr as $value){
				$empconscount[$i]['empid'] = $value['empid'];
				$empconscount[$i]['hcc'] = count($value['hconsidarr']);
				$empconscount[$i]['icc'] = count($value['iconsidarr']);
				$empconscount[$i]['occ'] = count($value['oconsidarr']);
				$empconscount[$i]['pcc'] = count($value['pconsidarr']);
				$i++;
			}
		...
		public function getempconsarr($clid, $locid, $sex, $arl, $arh, $year, $month){
			$ret = array();
			
			$conarr = $this->getempconarr($clid, $locid, $sex, $arl, $arh, $year, $month);
			
			$ret = $this->getempcons($conarr);
			
			return $ret;
		}
		
		public function getempcons($conarr){
			$ret = array();
			...
			foreach ($conarr as $value){
				...
				if ($dbhandle){
					...
					if ($dbfound){
						...
						if ($tex){
							...
							if ($result){
								if (mysql_num_rows($result) > 0){
									...
									$rex = array();
									while ($resarr = mysql_fetch_assoc($result)){
										if (in_array($resarr['conid'], $value['conidarr'])){
											$rex[$j] = $resarr['id'];
											$j++;
										}
									}
									$ret[$i]['hconsidarr'] = $rex;
								}
							}
						}
						$tablename1  = "icmain";
						...
						if ($tex1){
							$sql1 = "SELECT * FROM `$this->db`.`$tablename1`";
							...
							if ($result1){
								if (mysql_num_rows($result1) > 0){
									...
									while ($resarr1 = mysql_fetch_assoc($result1)){
										if (in_array($resarr1['conid'], $value['conidarr'])){
											$rey[$k] = $resarr1['id'];
											$k++;
										}
									}
									$ret[$i]['iconsidarr'] = $rey;
								}
							}
						}
						$tablename2  = "ocmain";
						...
						if ($tex2){
							$sql2 = "SELECT * FROM `$this->db`.`$tablename2`";
							...
							if ($result2){
								if (mysql_num_rows($result2) > 0){
									...
									while ($resarr2 = mysql_fetch_assoc($result2)){
										if (in_array($resarr2['conid'], $value['conidarr'])){
											$rez[$l] = $resarr2['id'];
											$l++;
										}
									}
									$ret[$i]['oconsidarr'] = $rez;
								}
							}
						}
						$tablename3  = "pcmain";
						$tex3 = $this->tex($tablename3);
						if ($tex3){
							$sql3 = "SELECT * FROM `$this->db`.`$tablename3`";
							$result3 = mysql_query($sql3, $dbhandle);
							if ($result3){
								if (mysql_num_rows($result3) > 0){
									$m = 0;
									$rew = array();
									while ($resarr3 = mysql_fetch_assoc($result3)){
										if (in_array($resarr3['conid'], $value['conidarr'])){
											$rew[$m] = $resarr3['id'];
											$m++;
										}
									}
									$ret[$i]['pconsidarr'] = $rew;
								}
							}
						}
					}
