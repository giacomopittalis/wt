<?php
	include 'dbclass.php';
    class unlock extends db{
    	
    	private function getlockedlist(){
    		$ret = array();
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					if ($tex){
						...
						$sql = "SELECT `$tablename`.`uname`,  `$tablename`.`fname`, `$tablename`.`lname` FROM `$this->db`.`$tablename` WHERE `$tablename`.`status1` = '$elocked'";
		...
		public function printlockedlist(){
			$res = $this->getlockedlist();
		...
		public function unlockuser($uname){
			$ret = FALSE;
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					if ($tex){
						...
						$sql1 = "UPDATE `$this->db`.`$tablename` SET `$tablename`.`status1` = '$eactive' WHERE `$tablename`.`uname` = '$euname'";
		...
		public function upunlocklog($suname, $ipadd, $dts, $uname){
			$ret = FALSE;
			$dbhandle = $this->dbhandle();
			if ($dbhandle){
				$dbfound = $this->dbfound();
