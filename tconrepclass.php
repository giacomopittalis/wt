<?php
    ...
    class tconrep extends db{
		...
		public function getconscount($clid, $locid, $sex, $arl, $arh, $year, $month){
			$ret = array();
			
			$hcon = $this->getallhcon();
			$ret[0] = $this->getconcount($clid, $locid, $sex, $arl, $arh, $year, $month, $hcon);
			$icon = $this->getallicon();
			$ret[1] = $this->getconcount($clid, $locid, $sex, $arl, $arh, $year, $month, $icon);
			$ocon = $this->getallocon();
			$ret[2] = $this->getconcount($clid, $locid, $sex, $arl, $arh, $year, $month, $ocon);
			$pcon = $this->getallpcon();
			$ret[3] = $this->getconcount($clid, $locid, $sex, $arl, $arh, $year, $month, $pcon);
			
			return $ret;
		}
		
		public function getconcount($clid, $locid, $sex, $arl, $arh, $year, $month, $con){
		...
		public function getallhcon(){
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					$tex = $this->tex($tablename);
					if ($tex){
						$sql = "SELECT `$tablename`.`conid` FROM `$this->db`.`$tablename`";
						$result = mysql_query($sql, $dbhandle);
						if ($result){
							if (mysql_num_rows($result) > 0){
								$i = 0;
								while ($resarr = mysql_fetch_assoc($result)){
									$ret[$i]['conid'] = $resarr['conid'];
									$i++;
								}
							}
						}
