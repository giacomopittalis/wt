    ...
    class mcconrep extends db{
		...
		public function printlinkedclientlist(){
			...
			if (empty($res)){
				...
				print "<select name=\"lc[]\" id=\"sel1\" tabindex=\"1\" multiple = \"multiple\">";
					print "<option value =\"0\" selected=\"selected\">All Clients</option>";
		...
		public function getmcconrep($clid, $locid, $sex, $arl, $arh, $year, $month){
			$ret = array();
			$i = 0;
			foreach ($clid as $value){
				$ret[$i] = $this->getmconrep($value, $locid, $sex, $arl, $arh, $year, $month);
				$i++;
			}
			return $ret;
		}
		...
		public function getlegarr($clid){
			$ret = array();
			$i = 0;
			foreach ($clid as $value){
				$ret[$i] = $this->getclname($value);
				$i++;
			}
			return $ret;
		}
		
		public function getallclid(){
			$ret = array();
			$active = "active";
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					$tex = $this->tex($tablename);
					if ($tex){
						$sql = "SELECT `$tablename`.`id` FROM `$this->db`.`$tablename` WHERE `$tablename`.`status` = '$active'";
						$result = mysql_query($sql, $dbhandle);
						if ($result){
							if (mysql_num_rows($result) > 0){
								$i = 0;
