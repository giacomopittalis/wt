...
<head>
  ...
  <title>Wellness - Create New Topic</title>
  ...
  <?php
	...
	if (isset($_POST['sub1'])){
		...
		$topicsta = $obj1->vtopic($topic, $constypeid);
		if ($topicsta != ""){
			$e++;
		}
		...
		//All is well
		if ($e == 0){
			$res1 = $obj1->crtopic1($constypeid, $topic);
			if ($res1){
				...
				$topicid = $obj1->getlastinsertid();
				$res2 = $obj1->uptopiclog($uname, $ipadd, $dts, $action, $topicid);
				if ($res2){
					$outsta = "Topic created successfully";
				}
				else{
		...
		<div class="twelve columns" id="container1a">
			...
			<div class="row">
				<div class="twelve columns">
					<form name="frmtopic" id="topicfrom" action="crtopic.php" method="post">
						<fieldset>
							...
							<div class="row">
								...
								<div class="four columns">
									<input type="text" name="topic" id="txt1" tabindex="2" maxlength="90"/>
								</div>
								<div class="five columns">
									<?php print $topicsta; ?>
								</div>
