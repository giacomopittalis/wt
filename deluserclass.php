<?php
    ...
    class deluser extends db{
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					if ($tex){
						$deleted = "deleted";
						$edeleted = $this->encode($deleted);
						$sql1 = "UPDATE `$this->db`.`$tablename` SET `$tablename`.`status1` = '$edeleted' WHERE `$tablename`.`uname` = '$euname'";
		...
		public function upuserdellog($suname, $ipadd, $dts, $uname){
			$ret = FALSE;
			$dbhandle = $this->dbhandle();
			if ($dbhandle){
				$dbfound = $this->dbfound();
