<?php
    ...
    class mlconrep extends db{
		...
		public function printlinkedclientlist(){
			...
			if (empty($res)){
				print "<select name=\"lc\" id=\"sel1\" tabindex=\"1\" >";
					print "<option value =\"0\">Select Client Clients</option>";
				print "</select>";
			}
			else{
		...
		public function printcurloclist($clid){
			...
			if ($clid == "0"){
				...
				if (empty($res)){
					...
					print "<select name=\"lid[]\" id=\"sel2\" tabindex=\"2\" multiple=\"multiple\">";
						print "<option value =\"0\" selected=\"selected\">All Locations</option>";
						foreach ($res as $value){
		...
		public function getmlconrep($clid, $locid, $sex, $arl, $arh, $year, $month){
			$ret = array();
			$i = 0;
			foreach ($locid as $value){
				$ret[$i] = $this->getmconrep($clid, $value, $sex, $arl, $arh, $year, $month);
				$i++;
			}
			return $ret;
		}
		...
		public function getalllocid($clid){
			$ret = array();
			$active = "active";
			$active = $this->encode($active);
			$dbhandle = $this->dbhandle();
			if ($dbhandle){
				$dbfound = $this->dbfound();
				if ($dbfound){
					...
					$tex = $this->tex($tablename);
					if ($tex){
						$sql = "SELECT `$tablename`.`id` FROM `$this->db`.`$tablename` WHERE `$tablename`.`clid` = '$clid' AND `$tablename`.`status` = '$active'";
						$result = mysql_query($sql, $dbhandle);
						if ($result){
							if (mysql_num_rows($result) > 0){
								$i = 0;
		...
		public function randcolorarr($locid){
			$ret = array();
			$count = count($locid);
			for ($i = 0; $i <= ($count - 1); $i++){
		...
		public function getlegarr($locid){
			$ret = array();
			$i = 0;
			foreach ($locid as $value){
				$ret[$i] = $this->getlocname($value);
				$i++;
			}
			return $ret;
		}
