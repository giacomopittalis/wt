<?php
	...
	class croconsult extends db{
		...
		public function insocmain($conid){
			$ret = FALSE;
		...
		public function insoccomm($ocid, $comm, $fuc, $fun){
			$ret = FALSE;
		...
		public function insocdump($ocid, $comm, $fuc, $fun){
			$ret = FALSE;
			
			$res1 = $this->insoccomm($ocid, $comm, $fuc, $fun);
			if ($res1){
				$ret = TRUE;
			}
			return $ret;
		...
		public function croc($input){
			$ret = FALSE;
			
			$res1 = $this->insocmain($input['conid']);
			if ($res1){
				$ocid = $this->getlastinsertid();
				$res2 = $this->insocdump($ocid, $input['comm'], $input['fuc'], $input['fun']);
				if ($res2){
					$res3 = $this->upoclog($ocid);
					if ($res3){
						$ret = TRUE;
					}
				}
