    ...
    class delfield extends db{
		...
		public function printsubtopiclist($constypeid, $topicid){
			if ($constypeid == "0" || $topicid == "0"){
				print "<select name=\"subtopic\" id=\"sel3\" tabindex=\"3\" onchange=\"getfield();\">";
		...
		public function getfieldlist($constypeid, $topicid, $subtopicid){
			$ret = array();
		...
		public function printfieldlist($constypeid, $topicid, $subtopicid){
			if ($constypeid == "0" || $topicid == "0" || $subtopicid == "0"){
				print "<select name=\"field\" id=\"sel4\" tabindex=\"4\">";
					print "<option value=\"0\">Select Field</option>";
				print "</select>";
			}
			else{
				$res = $this->getfieldlist($constypeid, $topicid, $subtopicid);
				if (empty($res)){
					print "<select name=\"field\" id=\"sel4\" tabindex=\"4\">";
						print "<option value=\"0\">Select Field</option>";
					print "</select>";
					...
					print "<select name=\"field\" id=\"sel4\" tabindex=\"4\">";
						print "<option value=\"0\">Select Field</option>";
						foreach ($res as $value){
							print "<option value=\"".$value['id']."\">".$value['fldname']."</option>";
						}
					print "</select>";
				}
			}
		...
		public function printinitfieldlist(){
			if (isset($_SESSION['constype']) && $_SESSION['constype'] != "" && $_SESSION['constype'] != "0"){
				$constypeid = $_SESSION['constype'];
			}
			else{
		...
		public function vfieldid($fieldid){
			$ret = "";
			
			if ($fieldid == "0"){
				$ret = "Please select a field";
			}
			return $ret;
		}
		
		public function delfield1($fieldid){
			$ret = FALSE;
			...
			if ($dbhandle){
				...
				if ($dbfound){
					...
					$tex = $this->tex($tablename);
					if ($tex){
						$sql = "UPDATE `$this->db`.`$tablename` SET `$tablename`.`status` = '$deleted' WHERE `$tablename`.`id` = '$fieldid'";
						$result = mysql_query($sql, $dbhandle);
						if ($result){
							$ret = TRUE;
						}
